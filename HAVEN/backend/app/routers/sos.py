...
async def _timer_countdown(user_id: str, duration_minutes: int):
    ...
    try:
        ...
        # We need to simulate a request. For this MVP, await trigger_sos.
        await trigger_sos(SOSRequest(), user_id)
...
async def trigger_sos(
    ...
    # Emit Socket.IO alert to responders (best-effort)
    try:
    ...
    try:
        ...
        # don't break the HTTP response for emit failures
        pass
...
async def update_live_location(
    ...
    # Emit live location update to responders and the user room
    try:
        await sio.emit("sos_location_update", {
            "sos_id": sos_id,
            "user_id": user_id,
